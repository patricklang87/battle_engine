...
const beverly = {
  ...
  mp: 15,
  maxMP: 15,
  prevImg: "resources/images/beverlyFcRight.jpg",
  attacks: {
    punch: {
      name: "Punch",
      freq: 5,
...
const battleBot = {
  ...
  mp: 10,
  maxMP: 10,
  prevImg: "resources/images/battleBotDisImg.jpg",
  attacks: {
    zapper: {
      name: "Zapper",
      freq: 5,
...
const wildDogs = {
  ...
  mp: 10,
  maxMP: 10,
  prevImg: "resources/images/wildDogsDisImg.jpg",
  attacks: {
    pounce: {
      name: "Pounce",
      freq: 5,
...
const changeSelButtonColor = (obj) => {
  obj.style.backgroundColor = "hsla(180, 10%, 70%, 1)";
}

const changeBackSelButtonColor = (obj) => {
    obj.style.backgroundColor = "hsla(180, 10%, 80%, 1)";
}

const changeBatButtonColor = (obj) => {
  obj.style.backgroundColor = "hsla(360, 90%, 40%, 1)";
}

const changeBackBatButtonColor = (obj) => {
    obj.style.backgroundColor = "hsla(360, 90%, 50%, 1)";
}
...
const disPrevFig2 = (img) => {
  img.transform = "scaleX(-1)";
  document.getElementById("imgDiv2").src = img;
}

const hidePrevFig2 = () => {
  document.getElementById("imgDiv2").src = "";
}
...
const createHeroSelect = () => {
  ...
  let figSelButtons = "";
  for (let i in figAr) figSelButtons = figSelButtons.concat(`<button id="${figAr[i]}Sel" class="setUpButton" onmouseover="disPrevFig(${figAr[i]}.prevImg); changeSelButtonColor(this)" onmouseout="hidePrevFig(); changeBackSelButtonColor(this)" onclick="heroSelect(${figAr[i]})";}>${figAr[i]}</button>`);
...
const heroSelect = (selFig) => {
  ...
  for (let i in enemyAr) enemySelButtons = enemySelButtons.concat(`<button id="${enemyAr[i]}Sel" class="setUpButton" onmouseover="disPrevFig2(${enemyAr[i]}.prevImg); changeSelButtonColor(this)" onmouseout="hidePrevFig2(); changeBackSelButtonColor(this)" onclick="enemySel(${hero}, ${enemyAr[i]})";}>${enemyAr[i]}</button>`);
...
disBatOps = (hero, enemy) => {
  ...
  var buttonAr = [];
  for (let i in heroOps) buttonAr.push(`<button id="${heroOps[i]}" onmouseover="changeBatButtonColor(this)" onmouseout="changeBackBatButtonColor(this)" value="${heroOps[i]}" class="batOpButton" onclick="heroAttacks(${hero.callName}, ${enemy.callName}, this.value)">${heroOps[i]}</button>`)
  var buttonString = "";
