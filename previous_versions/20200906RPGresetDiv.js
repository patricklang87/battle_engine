...
//the right set up needs to be found to make figures images automatically appear
...
let memAr = [];
let figAr = ["beverly", "battleBot", "wildHounds"];
...
function toggleHidden(item) {
 ...
 const disContSequence = (hero, enemy) => {
 ...
 const displayStatus = (hero, enemy) => {
...
const disFigSelImg = (fig) => {
  let img = fig.callName;
  document.getElementById("imgDiv").src = `${img}Img.jpg`;
}
...
const createHeroSelect = () => {
  ...
  let figSelButtons = "";
  for (let i in figAr) figSelButtons = figSelButtons.concat(`<button id="${figAr[i]}Sel" class="setUpButton" onmouseover="disFigSelImg(${figAr[i]})" onclick="heroSelect(${figAr[i]})";}>${figAr[i]}</button>`);
  document.getElementById("controlButtons").innerHTML = figSelButtons;
  document.getElementById("resetDiv").innerHTML = `<button id="resetPage" class="setUpButton" onclick="resetPage(${hero}, ${enemy})">Reset</button>`;
}
...
const beginBattle = (hero, enemy) => {
  ...
  if (hero.hp <= 0) disBatInfo(`${hero.disName} is already dead!`);
    ...
    disBatInfo(`${hero.disName} is treacherously ambushed by ${enemy.disName}!`);
    ...
    disBatInfo(`${hero.disName} encounters ${enemy.disName}!`);
    let turnAr = [hero.disName, enemy.disName];
    memAr.push(turnAr[turnAr.length * Math.floor(Math.random())]);
    disContSequence(hero, enemy);
  }
}


const contSequence = (hero, enemy) => {
   if (hero.hp <=0) disBatInfo("You lose!");
   else if (enemy.hp <=0) disBatInfo("You win!");
   else if (memAr[memAr.length - 1] == enemy.disName || memAr.length == 0) {
